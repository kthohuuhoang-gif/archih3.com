...
Phaser.Scene.prototype.showInstructionPopup = function (title, lines, onStart) {
    ...
    let box = this.add.rectangle(640, 360, 850, 450, AH3_COLORS.bgMedium.num, 0.85).setStrokeStyle(4, AH3_COLORS.warning.num).setDepth(POPUP_DEPTH);

    let t = this.add.text(640, 200, title, { fontFamily: FONT_TITLE, fontSize: '32px', fill: AH3_COLORS.danger.hex, fontStyle: 'bold', stroke: AH3_COLORS.bgDarkest.hex, strokeThickness: 4 }).setOrigin(0.5).setDepth(POPUP_DEPTH);
    let d = this.add.text(640, 320, lines.join('\n'), { fontFamily: FONT_MAIN, fontSize: '20px', fill: AH3_COLORS.textPrimary.hex, align: 'left', lineSpacing: 15 }).setOrigin(0.5).setDepth(POPUP_DEPTH);
...
class AH3_SceneIntro extends Phaser.Scene {
    ...
    create() {
        ...
        this.add.text(640, 130, 'MÀN 1: GÓC LÀM VIỆC LÚC 3 GIỜ SÁNG', { fontFamily: FONT_TITLE, fontSize: '24px', fill: AH3_COLORS.warning.hex, fontStyle: 'bold' }).setOrigin(0.5);
        ...
        box.setStrokeStyle(2, AH3_COLORS.warning.num);
        ...
        this.add.text(640, 280, instructions.join('\n'), { fontFamily: FONT_MAIN, fontSize: '20px', fill: AH3_COLORS.textSecondary.hex, align: 'left', lineSpacing: 10 }).setOrigin(0.5);
        ...
        let createJumpBtn = (x, text, targetScene, setupStats) => {
            createRoundedButton(this, x, 575, 230, 34, text, {
                bgColor: AH3_COLORS.bgLightAlt.num,       // navy #34495e
        ...
        createJumpBtn(390, 'Màn 1.5 (Tẩy Xóa)', 'SceneCloneStamp', () => { AH3_GAME_STATS.hasEduWatermark = true; });
        ...
        this.add.text(510, 663, 'archih3.com  •  AH3 Hub v1.8', { fontFamily: FONT_MAIN, fontSize: '13px', fill: AH3_COLORS.textMuted.hex }).setOrigin(0, 0.5);
...
class AH3_Scene1 extends Phaser.Scene {
    ...
    checkGameover() {
        if ((AH3_GAME_STATS.stressLevel >= 100 || AH3_GAME_STATS.deadlineTime <= 0) && !this.isGameOver) {
            ...
            let box = this.add.rectangle(640, 370, 600, 420, AH3_COLORS.bgLight.num).setStrokeStyle(4, AH3_COLORS.danger.num);
            ...
            this.add.text(640, 280, reason, { fontFamily: FONT_MAIN, fontSize: '24px', fill: AH3_COLORS.textPrimary.hex, fontStyle: 'bold' }).setOrigin(0.5);
    ...
    triggerLicenseAudit() {
        ...
        let bg = this.add.rectangle(0, 0, 780, 320, AH3_COLORS.bgDark.num, 0.95).setStrokeStyle(2, AH3_COLORS.danger.num);
        ...
        let txtSub = this.add.text(0, -40, 'Hãng Ao-Tu-Đét phát hiện bạn xài bản Crack!\nXử lý ngay trong 5 giây nếu không muốn lên phường uống trà:', { fontFamily: FONT_MAIN, fontSize: '24px', fill: AH3_COLORS.textPrimary.hex, align: 'center', lineSpacing: 8 }).setOrigin(0.5);
    ...
    triggerCrashEvent() {
        ...
        let popup = this.add.container(640, 360);
        let bg = this.add.rectangle(0, 0, 560, 280, AH3_COLORS.bgDark.num, 0.95).setStrokeStyle(2, AH3_COLORS.info.num);

        let txtTitle = this.add.text(0, -80, '🔥 3D MẮC ĐÃ LĂN ĐÙNG RA CHẾT 🔥', { fontFamily: FONT_MAIN, fontSize: '32px', fill: AH3_COLORS.danger.hex, fontStyle: 'bold', stroke: AH3_COLORS.bgDarkest.hex, strokeThickness: 4 }).setOrigin(0.5);
        let txtSub = this.add.text(0, -30, 'Máy treo cứng! Bấm điên cuồng để kích hoạt Auto-Backup trước khi màn hình xanh:', { fontFamily: FONT_MAIN, fontSize: '24px', fill: AH3_COLORS.textPrimary.hex }).setOrigin(0.5);

        let btnSave = this.add.rectangle(0, 50, 350, 70, AH3_COLORS.warning.num).setInteractive();
        ...
        let timer = 5;
        let timerTxt = this.add.text(0, 120, `⏳ ${timer}s`, { fontFamily: FONT_MAIN, fontSize: '24px', fill: AH3_COLORS.danger.hex, fontStyle: 'bold' }).setOrigin(0.5);
...
class AH3_Scene2 extends Phaser.Scene {
    ...
    create() {
        ...
        this.bubbleText = this.add.text(640, 280, '', { fontFamily: FONT_MAIN, fontSize: '24px', fill: AH3_COLORS.bgDark.hex, align: 'center', fontStyle: 'bold', lineSpacing: 10 }).setOrigin(0.5);
    ...
    endScene(reason, nextScene) {
        ...
        this.add.rectangle(640, 260, 1000, 200, AH3_COLORS.bgMedium.num, 0.85).setStrokeStyle(3, AH3_COLORS.danger.num);
        this.add.text(640, 260, reason, { fontFamily: FONT_TITLE, fontSize: '36px', fill: AH3_COLORS.danger.hex, fontStyle: 'bold', align: 'center', lineSpacing: 10, stroke: AH3_COLORS.bgDarkest.hex, strokeThickness: 4 }).setOrigin(0.5);
...
class AH3_SceneRageMode extends Phaser.Scene {
    ...
    create() {
        ...
        // --- TITLE ---
        let title = this.add.text(640, 60, '🔥 RAGE MODE - TRẢ THÙ! 🔥', { fontFamily: FONT_TITLE, fontSize: '48px', fill: AH3_COLORS.danger.hex, fontStyle: 'bold', align: 'center', stroke: AH3_COLORS.bgDarkest.hex, strokeThickness: 8 }).setOrigin(0.5);
        this.tweens.add({ targets: title, scaleX: 1.05, scaleY: 1.05, duration: 100, yoyo: true, repeat: -1 });

        // --- DESTRUCTION BAR ---
        let hudBox = this.add.rectangle(640, 150, 700, 80, AH3_COLORS.bgDark.num, 0.7).setStrokeStyle(2, AH3_COLORS.danger.num);
        this.add.text(640, 130, 'DESTRUCTION', { fontFamily: FONT_MAIN, fontSize: '20px', fill: AH3_COLORS.danger.hex, fontStyle: 'bold' }).setOrigin(0.5);
        this.add.rectangle(640, 165, 600, 30, AH3_COLORS.strokeDark.num, 0.7).setStrokeStyle(2, AH3_COLORS.textPrimary.num);
        this.barDestruction = this.add.rectangle(340, 165, 0, 30, AH3_COLORS.danger.num).setOrigin(0, 0.5);
        this.txtDestruction = this.add.text(940, 165, '0%', { fontFamily: FONT_MAIN, fontSize: '24px', fill: AH3_COLORS.textPrimary.hex, fontStyle: 'bold' }).setOrigin(1, 0.5);

        // --- INTERACTIVE AREA (SMASH) ---
        let smashArea = this.add.rectangle(640, 450, 1280, 500, AH3_COLORS.danger.num, 0).setInteractive();
    ...
    createSkillBtn(x, y, text, color) {
        let btn = this.add.rectangle(x, y, 120, 120, AH3_COLORS.bgDark.num, 0.75).setInteractive();
        btn.setStrokeStyle(3, color);
        this.add.text(x, y, text, { fontFamily: FONT_MAIN, fontSize: '24px', fill: AH3_COLORS.textPrimary.hex, align: 'center', fontStyle: 'bold' }).setOrigin(0.5);

        btn.on('pointerover', () => btn.setFillStyle(AH3_COLORS.strokeDark.num));
        btn.on('pointerout', () => btn.setFillStyle(AH3_COLORS.bgDark.num));
        return btn;
    }
    ...
    winGame() {
        let overlay = this.add.rectangle(640, 360, 1280, 720, AH3_COLORS.bgDarkest.num, 0.95).setInteractive();
        this.add.text(640, 200, '🏢 VĂN PHÒNG ĐÃ THÀNH ĐỐNG TRO TÀN!', { fontFamily: FONT_MAIN, fontSize: '48px', fill: AH3_COLORS.danger.hex, align: 'center', fontStyle: 'bold', lineSpacing: 20 }).setOrigin(0.5);
        this.add.text(640, 280, 'Bạn đã được giải thoát khỏi kiếp làm thuê.', { fontFamily: FONT_MAIN, fontSize: '24px', fill: AH3_COLORS.textPrimary.hex, align: 'center' }).setOrigin(0.5);
...
class AH3_SceneCloneStamp extends Phaser.Scene {
    ...
    create() {
        ...
        this.add.text(winX - winWidth / 2 + 15, titleBarY, 'Ps Adobe Photoshop 2026 (Unlicensed) - Render_Cuoi_Cung_Chot_V9.jpg', { fontFamily: FONT_MAIN, fontSize: '16px', fill: AH3_COLORS.textSecondary.hex }).setOrigin(0, 0.5);
        ...
        this.timeLeft = 15;
        this.txtTimer = this.add.text(wsCenterX, workspaceTop + 140, '⏳ 15s', { fontFamily: FONT_MAIN, fontSize: '48px', fill: AH3_COLORS.warning.hex, fontStyle: 'bold', stroke: AH3_COLORS.bgDarkest.hex, strokeThickness: 4 }).setOrigin(0.5);
...
class AH3_SceneDebtRunner extends Phaser.Scene {
    ...
    create() {
        ...
        this.warningQueue = [];
        this.warningPanelBg = this.add.rectangle(180, 115, 290, 145, AH3_COLORS.bgDarkest.num, 0.75).setStrokeStyle(2, AH3_COLORS.warning.num).setDepth(20);
        this.warningTitle = this.add.text(180, 65, '⚠ BẪY SẮP TỚI', { fontFamily: FONT_MAIN, fontSize: '20px', fill: AH3_COLORS.warning.hex, fontStyle: 'bold' }).setOrigin(0.5).setDepth(21);
        // Text slots: align trái (so với icon bên trái)
        this.warningSlots = [
            this.add.text(90, 100, '', { fontFamily: FONT_MAIN, fontSize: '16px', fill: AH3_COLORS.textPrimary.hex, fontStyle: 'bold', align: 'left' }).setOrigin(0, 0.5).setDepth(21),
            this.add.text(90, 130, '', { fontFamily: FONT_MAIN, fontSize: '13px', fill: AH3_COLORS.strokeLight.hex, align: 'left' }).setOrigin(0, 0.5).setDepth(21),
            this.add.text(90, 158, '', { fontFamily: FONT_MAIN, fontSize: '13px', fill: AH3_COLORS.textMuted.hex, align: 'left' }).setOrigin(0, 0.5).setDepth(21)
        ];
    ...
    collectCoin(player, coin) {
        ...
        let f = this.add.text(this.player.x, this.player.y - 100, '+ HEO HÚC!', { fontFamily: FONT_MAIN, fontSize: '24px', fill: AH3_COLORS.info.hex, fontStyle: 'bold', stroke: AH3_COLORS.textPrimary.hex, strokeThickness: 4 }).setOrigin(0.5);
        this.tweens.add({ targets: f, y: f.y - 100, alpha: 0, duration: 1000, onComplete: () => f.destroy() });
    }
    ...
    showGameOverPopup(msg) {
        ...
        let box = this.add.rectangle(640, 360, 650, 360, AH3_COLORS.bgDarkest.num, 0.95).setStrokeStyle(4, AH3_COLORS.danger.num).setDepth(POPUP_DEPTH + 1);
        ...
        if (reason) {
            this.add.text(640, 300, reason, { fontFamily: FONT_MAIN, fontSize: '24px', fill: AH3_COLORS.textPrimary.hex, align: 'center', fontStyle: 'bold', lineSpacing: 10 }).setOrigin(0.5).setDepth(POPUP_DEPTH + 1);
        }
    ...
    showWinPopup() {
        ...
        const POPUP_DEPTH = 100;
        this.add.rectangle(640, 360, 1280, 720, AH3_COLORS.bgDarkest.num, 0.6).setDepth(POPUP_DEPTH);
        let box = this.add.rectangle(640, 360, 800, 300, AH3_COLORS.bgDarkest.num, 0.95).setStrokeStyle(4, AH3_COLORS.warning.num).setDepth(POPUP_DEPTH + 1);

        this.add.text(640, 250, 'BẠN ĐÃ ĐUỔI KỊP SHARK LƯƠN!', { fontFamily: FONT_TITLE, fontSize: '48px', fill: AH3_COLORS.warning.hex, fontStyle: 'bold', stroke: AH3_COLORS.bgDarkest.hex, strokeThickness: 5 }).setOrigin(0.5).setDepth(POPUP_DEPTH + 1);
        this.add.text(640, 310, 'Nhưng hắn vừa leo lên chiếc Maybach rồ ga chạy mất.\nTiền tài khoản của bạn: -500.000 VNĐ', { fontFamily: FONT_MAIN, fontSize: '24px', fill: AH3_COLORS.textPrimary.hex, align: 'center', fontStyle: 'bold', lineSpacing: 10 }).setOrigin(0.5).setDepth(POPUP_DEPTH + 1);
...
(function () {
    ...
    // Hub WebView2 trên Windows desktop → isTouchDevice = false → skip toàn bộ
