...
// ==========================================
// AH3 BGM SYSTEM (2026-05-19) - Phaser Audio + Real MP3 Files
// ==========================================
// REFACTOR v2: Bỏ Tone.js procedural (sai vibe, gây mệt). Chuyển sang real MP3
// từ Pixabay (free commercial, no attribution required).
//
// Track Màn 1: viacheslavstarostin "Gaming Game Video Game Music" (Pixabay 474517)
//   - 120s loop, 89 BPM, G# minor, balanced timbre, dense melody
//   - Mood: chillhop gaming vibe, không mệt, không buồn
//
// Architecture:
//   - Audio file preload trong Scene preload() (Phaser cache)
//   - play(scene, key) start với loop:true, fade in 1s
//   - stop() stop instant (scene shutdown)
//   - localStorage 'ah3_bgm_muted' persist mute state
//   - Phaser tự manage iOS audio unlock (đã hoạt động cho SFX)
// ==========================================
const AH3_BGM = {
    currentKey: null,       // key đang play (null = silent)
    currentSound: null,     // Phaser Sound instance
    muted: false,           // user mute state
    volume: 0.4,            // default volume (0.0-1.0). 0.4 ≈ -8dB

    // Init: đọc mute preference từ localStorage. Gọi 1 lần đầu game.
    init() {
        try {
            this.muted = (localStorage.getItem('ah3_bgm_muted') === '1');
        } catch (e) { /* private mode block localStorage */ }
        console.log('[AH3_BGM] Initialized. Muted:', this.muted);
    },

    // Play BGM theo key. Tự stop track cũ trước. Fade in 1s.
    play(scene, key) {
        this.init();
        if (this.currentKey === key && this.currentSound && this.currentSound.isPlaying) return;

        // Stop track cũ nếu có
        this.stop();

        // Verify scene + audio available
        if (!scene || !scene.sound || !scene.cache.audio.exists(key)) {
            console.warn('[AH3_BGM] Audio key not loaded:', key);
            return;
        }
        ...
        try {
            ...
            this.currentKey = key;
            console.log('[AH3_BGM] Playing:', key);
        } catch (e) {
            console.error('[AH3_BGM] Play failed:', e);
        }
    },

    // Stop BGM hiện tại (instant — không fade vì scene đang shutdown)
    stop() {
    ...
    // Toggle mute (cho UI button sau)
    toggleMute() {
        this.muted = !this.muted;
        try { localStorage.setItem('ah3_bgm_muted', this.muted ? '1' : '0'); } catch (e) {}
        if (this.currentSound) {
            this.currentSound.setVolume(this.muted ? 0 : this.volume);
        }
        return this.muted;
    }
};
...
class AH3_Scene1 extends Phaser.Scene {
    ...
    preload() {
        ...
        // BGM MÀN 1: chillhop gaming track (Pixabay 474517, viacheslavstarostin)
        // 120s, 89 BPM, G# minor — vibe gaming vui vẻ, không mệt khi nghe nền
        if (!this.cache.audio.exists('bgm_scene1')) {
            this.load.audio('bgm_scene1', './assets/bgm_scene1.ogg');
        }
    }
    ...
    create() {
        ...
        // ==========================================
        // BGM MÀN 1: Chillhop gaming (Pixabay 474517) - 2026-05-19
        // ==========================================
        // Play khi vào scene, auto-stop khi shutdown (chuyển sang Scene CloneStamp hoặc Scene2)
        AH3_BGM.play(this, 'bgm_scene1');
        this.events.once('shutdown', () => AH3_BGM.stop());
        this.events.once('destroy',  () => AH3_BGM.stop());
    }
    ...
    activateRandomTool() {
        ...
        // RHYTHM-SYNC (2026-05-19): 4000ms → 2700ms để khớp BGM 89 BPM
        // 1 bar BGM = 4 beats × (60/89) = 2.697s ≈ 2.7s → click cycle = đúng 1 bar nhạc
        // Effect: gameplay feel sync với drum kick, người chơi vào "flow state" theo nhịp
        this.miniTimeMax = 2700;
    ...
    generateArrowSequence() {
        ...
        // LAYOUT-V3 (2026-05-19): 2-row STRICT keyboard layout
        //   - Row 1: TẤT CẢ mũi tên trên 1 hàng (size 45px, gap auto-shrink theo arrowLevel)
        //   - Row 2: SPACE bar 400×50 căn giữa
        // KHÔNG wrap arrows xuống 2 hàng — anh muốn arrows luôn 1 hàng để dễ đọc
        // Auto-shrink: arrows gap từ 60px (level 3) → 48px (level 10) để fit khung 700px
        const ARROW_SIZE = 45;
        const SPACE_WIDTH = 400;
        const SPACE_HEIGHT = 50;
        const ROW_GAP = 30;          // khoảng cách arrow row ↔ space row
        const MAX_TOTAL_WIDTH = 700; // khung chứa max

        // Auto-shrink gap: nếu level cao, arrows gần nhau hơn để vẫn 1 hàng
        // Total width arrows = (count - 1) * gap + ARROW_SIZE
        // Cần: total ≤ MAX_TOTAL_WIDTH
        let gap = Math.min(60, (MAX_TOTAL_WIDTH - ARROW_SIZE) / Math.max(1, this.arrowLevel - 1));
        let arrowY = -(ROW_GAP + SPACE_HEIGHT) / 2;  // arrow row ở trên
        let spaceY = arrowY + ARROW_SIZE / 2 + ROW_GAP + SPACE_HEIGHT / 2;
        let arrowStartX = -((this.arrowLevel - 1) * gap) / 2;

        // ROW 1: tất cả mũi tên trên 1 hàng
        for (let i = 0; i < this.arrowLevel; i++) {
            let dir = Phaser.Utils.Array.GetRandom(arrows);
            this.arrowSequence.push(dir);
            let x = arrowStartX + i * gap;
            let box = this.add.rectangle(x, arrowY, ARROW_SIZE, ARROW_SIZE, AH3_COLORS.bgMedium.num).setStrokeStyle(2, AH3_COLORS.strokeMid.num);
            let txt = this.add.text(x, arrowY, chars[dir], { fontSize: '28px' }).setOrigin(0.5);
            this.arrowSprites.push({ box: box, txt: txt });
            this.arrowContainer.add([box, txt]);
        }

        // ROW 2: SPACE bar 400px căn giữa
        this.arrowSequence.push('SPACE');
        let spaceBox = this.add.rectangle(0, spaceY, SPACE_WIDTH, SPACE_HEIGHT, AH3_COLORS.bgMedium.num).setStrokeStyle(2, AH3_COLORS.strokeMid.num);
        let spaceTxt = this.add.text(0, spaceY, 'SPACE', { fontFamily: FONT_MAIN, fontSize: '20px', fill: AH3_COLORS.textPrimary.hex, fontStyle: 'bold' }).setOrigin(0.5);
        this.arrowSprites.push({ box: spaceBox, txt: spaceTxt });
        this.arrowContainer.add([spaceBox, spaceTxt]);

        // Thời gian Audit (DIFFICULTY-V2 2026-05-19):
        // Plan A "Tight Casual": mỗi phím = 500ms + buffer 400ms. Khoảng 0.74 beat BGM/key.
        // Level 3 (4 keys) = 4·500 + 400 = 2400ms (600ms/key) — gay cấn ngay từ đầu
        // Level 10 (11 keys) = 11·500 + 400 = 5900ms (536ms/key) — đỉnh khó nhưng vẫn fair
        // Min cap 1500ms để tránh impossible nếu BGM/level thay đổi
        this.miniTimeMax = Math.max(1500, (this.arrowLevel + 1) * 500 + 400);
        this.miniTimeLeft = this.miniTimeMax;
        this.miniTimerActive = true;
    }
        ...
        if (pressedKey === expectedKey) {
            ...
            if (this.currentArrowIndex >= this.arrowSequence.length) {
                ...
                if (AH3_GAME_STATS.progress < 95) {
                    // RHYTHM-SYNC (2026-05-19): Cap 5 → 10 để game thật sự khó dần
                    // Level 3 (4 keys) → 10 (11 keys), mỗi lần audition tăng 1 level
                    if (this.arrowLevel < 10) this.arrowLevel++;
    ...
    triggerCrashEvent() {
        ...
        let txtSave = this.add.text(0, 60, '⚡ CLICK ĐIÊN CUỒNG ⚡\n(Còn 25 click)', { fontFamily: FONT_MAIN, fontSize: '20px', fill: AH3_COLORS.textPrimary.hex, align: 'center', fontStyle: 'bold' }).setOrigin(0.5);
        ...
        // CRASH-FRENZY (2026-05-19): 15 → 25 clicks/5s (5 click/s ≈ 0.3 beat BGM)
        // Bấm "điên cuồng" thật sự — phải dùng 2 ngón hoặc đập tay, fail rate ~30%
        // Drama narrative "VĂNG RỒI bay màu 50%" giờ có nguy cơ xảy ra → cay cú muốn chơi lại
        let clicksNeeded = 25;
        let countdown = this.time.addEvent({
            delay: 1000, repeat: 4,
            callback: () => {
...
class AH3_Scene2 extends Phaser.Scene {
    ...
    preload() {
        ...
        // BGM MÀN 2: Noir cuộc họp — 143 BPM F minor warm bass (Pixabay)
        if (!this.cache.audio.exists('bgm_scene2')) {
            this.load.audio('bgm_scene2', './assets/bgm_scene2.ogg');
        }
    }
    ...
    create() {
        ...
        // ==========================================
        // BGM MÀN 2: Noir cuộc họp hút máu (2026-05-19)
        // ==========================================
        AH3_BGM.play(this, 'bgm_scene2');
        this.events.once('shutdown', () => AH3_BGM.stop());
        this.events.once('destroy',  () => AH3_BGM.stop());
    }
...
class AH3_SceneRageMode extends Phaser.Scene {
    ...
    preload() {
        ...
        // BGM MÀN 4: Rage Mode đập phá — 143 BPM C minor balanced (Pixabay)
        if (!this.cache.audio.exists('bgm_scene4')) {
            this.load.audio('bgm_scene4', './assets/bgm_scene4.ogg');
        }
    }
    ...
    create() {
        ...
        // ==========================================
        // BGM MÀN 4: Rage Mode đập phá xả stress (2026-05-19)
        // ==========================================
        AH3_BGM.play(this, 'bgm_scene4');
        this.events.once('shutdown', () => AH3_BGM.stop());
        this.events.once('destroy',  () => AH3_BGM.stop());
    }
    ...
    winGame() {
        ...
        // (stats và maxCombo đã được khai báo ở đầu winGame() cho GA4 tracking — tái sử dụng tại đây)
...
class AH3_SceneCloneStamp extends Phaser.Scene {
    ...
    preload() {
        ...
        // BGM MÀN 1.5: TENSE 15s panic — 112 BPM D# minor (Pixabay)
        if (!this.cache.audio.exists('bgm_scene1_5')) {
            this.load.audio('bgm_scene1_5', './assets/bgm_scene1_5.ogg');
        }
    }
    ...
    create() {
        ...
        // ==========================================
        // BGM MÀN 1.5: TENSE panic 15s (2026-05-19)
        // ==========================================
        AH3_BGM.play(this, 'bgm_scene1_5');
        this.events.once('shutdown', () => AH3_BGM.stop());
        this.events.once('destroy',  () => AH3_BGM.stop());
    }
...
class AH3_SceneDebtRunner extends Phaser.Scene {
    ...
    preload() {
        // BGM MÀN 3: Chase đòi nợ — 108 BPM A# MAJOR BRIGHT (Pixabay)
        // Major key + bright = chase nhưng vui (giống Sonic), không stress
        if (!this.cache.audio.exists('bgm_scene3')) {
            this.load.audio('bgm_scene3', './assets/bgm_scene3.ogg');
        }
    }
    ...
    create() {
        ...
        // ==========================================
        // BGM MÀN 3: Chase đòi nợ (2026-05-19)
        // ==========================================
        AH3_BGM.play(this, 'bgm_scene3');
        this.events.once('shutdown', () => AH3_BGM.stop());
        this.events.once('destroy',  () => AH3_BGM.stop());
    }
